...
def main():
    ...
    if not ts_csv.exists():
        raise RuntimeError(f"Missing {ts_csv}. Run scripts/10_vault_timeseries.py first.")
    ...
    # 1) Token amounts over time (two axes)
    fig, ax1 = plt.subplots(figsize=(12.5, 6.0))
    ax2 = ax1.twinx()

    l1 = ax1.plot(df["dt"], df["amt_eth"], label="ETH amount", linewidth=1.8, color="#1f77b4")
    l2 = ax2.plot(df["dt"], df["amt_ixs"], label="IXS amount", linewidth=1.8, color="#ff7f0e")
    ...
    ax.stackplot(
        df["dt"],
        df["value_eth_usd"].astype(float),
        df["value_ixs_usd"].astype(float),
    ...
    # 3) Performance vs hold + full-range LP baseline (indexed)
    fig, ax = plt.subplots(figsize=(12.5, 6.0))
    ax.plot(df["dt"], df["vault_value_index"], label="Vault (index)", linewidth=2.2)
    ax.plot(df["dt"], df["hold_value_index"], label="Hold initial amounts (index)", linewidth=1.8)
    if "full_range_lp_value_index" in df.columns and df["full_range_lp_value_index"].notna().any():
        ax.plot(df["dt"], df["full_range_lp_value_index"], label="Full-range LP (no fees) baseline (index)", linewidth=1.8)
    ...
    # 4) Save a compact summary table (for report/client narrative)
    last = df.iloc[-1]
    summary = pd.DataFrame(
        [
            {
                "t0_datetime_utc": str(df.iloc[0]["datetime_utc"]),
                "t1_datetime_utc": str(last["datetime_utc"]),
                "vault_value_usd_t0": float(df.iloc[0]["value_total_usd"]),
                ...
                "full_range_lp_index_t1": float(last["full_range_lp_value_index"]) if np.isfinite(last.get("full_range_lp_value_index", np.nan)) else np.nan,
                "amt_eth_t0": float(df.iloc[0]["amt_eth"]),
                "amt_eth_t1": float(last["amt_eth"]),
                "amt_ixs_t0": float(df.iloc[0]["amt_ixs"]),
                "amt_ixs_t1": float(last["amt_ixs"]),
                "mapping_mode": str(df["underlying_mapping_mode"].mode().iloc[0]) if "underlying_mapping_mode" in df.columns else "",
            }
        ]
    )
